namespace IntegrationV2.Files.cs.Domains.MeetingDomain.Client
{
	...
	public class ExchangeCalendarClient: ICalendarClient
	{
		...
		private string GetOrganizerEmail(Appointment appointment) {
			return appointment.Organizer.Address.IsNullOrEmpty()
				? string.Empty
		...
		private List<MeetingDto> GetDeletedAppointments(Calendar calendar) {
			...
			foreach (var item in GetAppointmentsFromFolder(folder, changedItemsFilter)) {
				if (TryBindAppointment(item.Id.UniqueId, out Appointment appointment)) {
					_log.LogDebug($"Loaded meeting '{appointment.Subject}' for delete with " +
						$"ICalUid {GetICalUid(appointment)}");
					if (IsMeetingNotExistInCalendar(calendar, appointment) 
							&& TryConvertToMeetingDto(appointment, calendar, out var meetingDto)) {
						result.Add(meetingDto);
						_log.LogDebug($"Added meeting '{meetingDto.Title}' for delete with ICalUid {meetingDto.ICalUid}");
					}
				}
			}
			return result;
		...
		private bool IsMeetingNotExistInCalendar(Calendar calendar, Appointment appointment) {
			var calendarEmail = calendar.Settings.SenderEmailAddress;
			var requiredAttendee = appointment.RequiredAttendees.FirstOrDefault(
				a => a.Address.ExtractEmailAddress().Equals(calendarEmail, StringComparison.InvariantCultureIgnoreCase));
			var optionalAttendee = appointment.OptionalAttendees.FirstOrDefault(
				a => a.Address.ExtractEmailAddress().Equals(calendarEmail, StringComparison.InvariantCultureIgnoreCase));
			var notExistAttendee = requiredAttendee == null && optionalAttendee == null;
			return appointment.IsCancelled
				|| notExistAttendee
				|| optionalAttendee?.ResponseType == MeetingResponseType.Decline
				|| requiredAttendee?.ResponseType == MeetingResponseType.Decline;
		}
