namespace Terrasoft.IntegrationV2.Utils
{
	...
	internal class ListenerUtils
	{
		...
		/// <returns><c>True</c> if feature enabled, otherwise returns false.</returns>
		public static bool GetIsFeatureEnabled(UserConnection uc, string code, Guid sysAdminUnitId) {
			var featureUtil = ClassFactory.Get<IFeatureUtilities>();
			return featureUtil.GetIsFeatureEnabled(uc, code, sysAdminUnitId);
		}
		...
		/// <param name="sysAdminUnitId">SysAdminUnit unique identifier.</param>
		/// <returns><c>True</c> if feature disabled, otherwise returns false.</returns>
		public static bool GetIsFeatureDisabled(UserConnection uc, string code, Guid sysAdminUnitId) {
			return !GetIsFeatureEnabled(uc, code, sysAdminUnitId);
		}
